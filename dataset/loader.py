...
class PrefetchLoader2:
    ...
    def __iter__(self):
        ...
        for next_idx, next_input1, next_input2 in self.loader:

            with stream_context():
                next_input1 = next_input1.to(device=self.device, non_blocking=True)
                next_input2 = next_input2.to(device=self.device, non_blocking=True)
                next_idx = next_idx.to(device=self.device, non_blocking=True)
                next_input1 = next_input1.to(self.img_dtype).sub_(self.mean).div_(self.std)
                next_input2 = next_input2.to(self.img_dtype).sub_(self.mean).div_(self.std)
                if self.random_erasing is not None:
                    next_input1 = self.random_erasing(next_input1)
                    next_input2 = self.random_erasing(next_input2)

            if not first:
                yield next_idx, input1, input2
            else:
                first = False
