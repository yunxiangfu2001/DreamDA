...
from diffusers.utils import PIL_INTERPOLATION, deprecate, logging, randn_tensor, BaseOutput
...
class DDIMInversionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(self,
        ...
        synthetic_scale=10,):
        latents_noisy = self.diffusion_call(source_prompt='',
        ...
        imgs = self.diffusion_call(source_prompt='',
                                              prompt=prompt,
                                              image=image,
                                    reverse=False,
                                        fixed_starting_latent=latents_noisy,
                                        run_baseline=True,
                                        init_image_strength=init_image_strength,
                                        width=512,height=512,
                                        steps=100,
    ...
    def diffusion_call(
        ...
        if reverse:
            ...
            noise = torch.randn(clean_latents.shape,
                                generator=generator,
                                device=device,
                            dtype=torch.float64)
            if fixed_starting_latent is None:
                ...
                t_limit = num_inference_steps - int(num_inference_steps * init_image_strength)
        ...
        def image_to_latent(im):
            if isinstance(im, torch.Tensor):
                ...
                im = im.to(device)
                #Encode image
                init_latent = self.vae.encode(im).latent_dist.sample(generator=generator) * 0.18215
                return init_latent
        ...
        # timesteps = self.scheduler.timesteps[t_limit:]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            if reverse:
                for i, t in enumerate(timesteps):
                    ...
                    t_scale = t / self.scheduler.num_train_timesteps
                    ...
                    for latent_i in range(2): 
                        ...
                        noise_pred_uncond =  self.unet(latent_model_input, t, 
                                                encoder_hidden_states=source_prompt_embeds[0].unsqueeze(0),
                        ...
                        # noise_pred_cond = self.unet(latent_model_input, t, 
                        ...
                        #     use_last_tokens_attention_weights()
                            ...
                            noise_pred_cond = self.unet(latent_model_input,
                                                t, 
                                                encoder_hidden_states=prompt_embeds[1].unsqueeze(0),generator=generator).sample
                        ...
                        new_latent = step_call(self.scheduler,
                                                noise_pred,
                                                    ...
                                                    latent_base,
                                                    generator=generator,
                                                    iteration=i)# .prev_sample
                        new_latent = new_latent.to(latent_base.dtype)

                        latent_pair[latent_i] = new_latent
                return latent_pair[0]


            else:
                c, h ,w = latent_pair[0].size()
                for noise_i in range(synthetic_scale):
                    if isinstance(latent, list): # initializing from pair of images
                        latent_pair = latent
                    else: # initializing from noise
                        latent_pair = [latent.clone(), latent.clone()]
                    for i, t in enumerate(timesteps):
                        ...
                        for latent_i in range(2): 
                            ...
                            #Predict the unconditional noise residual
                            noise_pred_uncond =  self.unet(latent_model_input.view(1,c,h,w), t, 
                                                    encoder_hidden_states=source_prompt_embeds[0].unsqueeze(0),
                            ...
                            #Predict the conditional noise residual and save the cross-attention layer activations
                            noise_pred_cond = self.unet(latent_model_input.view(1,c,h,w), t, 
                    ...
                    all_latents.append(latent_pair[0])
